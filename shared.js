...
if (!_languageCode) {
    _languageCode = browser.i18n.getUILanguage();
...
function showIcon() {
    ...
    browser.browserAction.setTitle({ title: tipLines.join('\n') });
...
function startGetLocationName() {
    ...
    var url = 'https://maps.googleapis.com/maps/api/geocode/json?latlng={0},{1}&language={2}&key={3}'
        .filledWith(localStorage.lat, localStorage.long, browser.runtime.getManifest().current_locale,
...
function prepareAnalytics() {
    ...
    //    tracker.set('appVersion', browser.runtime.getManifest().version);
